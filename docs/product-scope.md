...
- Home: lead with native currency names and symbols (e.g. Brazilian real / R$), with the token/issuer secondary in details and transaction review. Your money splits stablecoins into Cash and all other tokens into Investments; Morpho vault positions appear only under Save. Add money, send, receive, and activity. Converted portfolio values carry an estimate label and price timestamp.
