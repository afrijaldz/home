...
6. **Walk the flow only with operator authorization.** Add money → select the configured deposit method → complete KYC when requested → review the quote → create the order → follow its instructions → watch Activity reach the provider-reported state and, after exact Base receipt evidence, `received`. `POST /api/funding/webhooks/ripio` cannot reach a local run without an external tunnel; local testing still progresses through status polling (the client polls every four seconds and the core limits refreshes to one per three seconds). Do not make a payment without explicit funded-test approval.
