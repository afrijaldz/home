...
5. **Confirm eligibility and run synthetic checks.** With every variable declared by the selected manifest binding set, **Add money** shows that binding for its country. A listed binding means the variables are set, not that their values are valid: discovery makes no provider call, so a wrong credential is first rejected at **Confirm** in step 6, which is also the first request to the provider. From the repository root run:
